import { UMB_THEME_CONTEXT as _ } from "@umbraco-cms/backoffice/themes";
const B = ["❄", "❅", "❆"], $ = 1e3 / 24;
class tt {
  #t = null;
  #e = null;
  #n = null;
  #s = [];
  #i = 0;
  #r = 0;
  #a = null;
  #o = null;
  #l = null;
  COUNT = 75;
  start(t, e) {
    this.#a = t ?? null;
    const s = e ?? document.body;
    if (this.#t = document.createElement("canvas"), this.#t.id = "umb-christmas-snow-canvas", Object.assign(this.#t.style, {
      position: "fixed",
      top: "0",
      left: "0",
      width: "100vw",
      height: "100vh",
      pointerEvents: "none",
      zIndex: "1"
    }), s.appendChild(this.#t), this.#e = this.#t.getContext("2d", { willReadFrequently: !1 }), !this.#e) {
      this.#t.remove(), this.#t = null, this.#u();
      return;
    }
    this.#f(), this.#A(), window.addEventListener("resize", this.#h), this.#i = performance.now(), this.#m();
  }
  stop() {
    this.#n !== null && cancelAnimationFrame(this.#n), window.removeEventListener("resize", this.#h), this.#t?.remove(), this.#t = null, this.#e = null, this.#a = null, this.#o?.remove(), this.#o = null, this.#l?.remove(), this.#l = null;
  }
  #A() {
    const t = innerWidth, e = innerHeight;
    this.#s = Array.from(
      { length: this.COUNT },
      () => this.#c(t, e, !0)
    );
  }
  #c(t, e, s = !1) {
    const n = 6 + Math.random() * 8;
    return {
      x: Math.random() * t,
      y: s ? Math.random() * e : -(10 + Math.random() * 40),
      char: B[Math.floor(Math.random() * B.length)],
      size: n,
      speed: 0.3 + Math.random() * 1.2,
      alpha: 0.3 + Math.random() * 0.7,
      wobblePhase: Math.random() * Math.PI * 2,
      wobbleAmp: 20 + Math.random() * 40,
      rotation: Math.random() * Math.PI * 2,
      rotSpeed: (Math.random() - 0.5) * 0.02
    };
  }
  #h = () => this.#f();
  #f() {
    if (!this.#t || !this.#e) return;
    const t = devicePixelRatio || 1, e = innerWidth, s = innerHeight;
    this.#t.width = e * t, this.#t.height = s * t, this.#t.style.width = e + "px", this.#t.style.height = s + "px", this.#e.setTransform(t, 0, 0, t, 0, 0);
  }
  #m = (t) => {
    this.#n = requestAnimationFrame((s) => this.#m(s));
    const e = t ?? performance.now();
    e - this.#r < $ || (this.#r = e, this.#d(), this.#V());
  };
  #d() {
    const t = innerWidth, e = innerHeight, s = (performance.now() - this.#i) / 1e3;
    for (const n of this.#s)
      n.y += n.speed, n.x += Math.sin(s * 0.5 + n.wobblePhase) * n.wobbleAmp * 0.01, n.rotation += n.rotSpeed, n.y > e + 30 && (this.#a?.(n.x), n.y = -(10 + Math.random() * 40), n.x = Math.random() * t), n.x < -30 && (n.x = t + 30), n.x > t + 30 && (n.x = -30);
  }
  #V() {
    const t = this.#e;
    if (t) {
      t.clearRect(0, 0, innerWidth, innerHeight), t.textAlign = "center", t.textBaseline = "middle";
      for (const e of this.#s)
        t.save(), t.translate(e.x, e.y), t.rotate(e.rotation), t.globalAlpha = e.alpha, t.font = `${e.size}px sans-serif`, t.fillStyle = "#ffffff", t.fillText(e.char, 0, 0), t.restore();
    }
  }
  #u() {
    this.#l = document.createElement("style"), this.#l.textContent = `
      @keyframes umb-snow{0%{transform:translateY(-20px) rotate(0deg);opacity:1}50%{transform:translateY(50vh) translateX(30px) rotate(180deg)}100%{transform:translateY(100vh) translateX(-20px) rotate(360deg);opacity:.3}}
      .umb-css-snow{position:fixed;top:-20px;pointer-events:none;z-index:99999;color:white;animation:umb-snow linear infinite;text-shadow:0 0 4px rgba(255,255,255,.6)}`, document.head.appendChild(this.#l), this.#o = document.createElement("div"), Object.assign(this.#o.style, {
      position: "fixed",
      inset: "0",
      pointerEvents: "none",
      zIndex: "99999",
      overflow: "hidden"
    }), document.body.appendChild(this.#o);
    for (let t = 0; t < 60; t++) {
      const e = document.createElement("span");
      e.className = "umb-css-snow", e.textContent = B[Math.floor(Math.random() * B.length)];
      const s = 8 + Math.random() * 8, n = 6 + Math.random() * 12;
      e.style.cssText = `left:${Math.random() * 100}%;font-size:${s}px;opacity:${0.3 + Math.random() * 0.7};animation-duration:${n}s;animation-delay:-${Math.random() * n}s`, this.#o.appendChild(e);
    }
  }
}
const d = 220, v = "data:image/png;base64,", et = `
#umb-christmas-santa-path {
  position: fixed;
  bottom: 0;
  left: auto;
  margin-top: -85px;
  animation: umb-santa-path-animation 10s infinite;
  z-index: 100;
  pointer-events: none;
}
#umb-christmas-santa-path #santa {
  width: 85px;
  height: 85px;
  border: 1px solid transparent;
  background: var(--umb-santa-sprite) no-repeat left top;
  background-size: auto 85px;
  animation: umb-santa-animation 0.8s steps(12) infinite;
}
@keyframes umb-santa-animation {
  0% { background-position: 0 0; }
  100% { background-position: -1020px 0; }
}
@keyframes umb-santa-path-animation {
  0% { left: -5%; }
  100% { left: 120%; }
}
`, st = 255, nt = 255, ot = 255, R = d - 1, it = 6, rt = 50;
class at {
  #t = null;
  #e = null;
  #n = null;
  #s = 0;
  #i = null;
  #r = null;
  #a = null;
  #o = null;
  #l = null;
  #A = 0;
  /** Returns true if snow can be placed at (xi, row): on the ground or supported by snow below (and diagonals). */
  #c(t, e) {
    if (e >= R) return !0;
    const s = this.#i, n = this.#s, o = t + (e + 1) * n, r = t > 0 ? t - 1 + (e + 1) * n : o, h = t < n - 1 ? t + 1 + (e + 1) * n : o;
    return s[o] === 1 && s[r] === 1 && s[h] === 1;
  }
  /** Try to place one pixel at column xi; returns true if placed. */
  #h(t) {
    if (!this.#i || !this.#r || this.#s <= 0) return !1;
    const e = Math.max(0, Math.min(this.#s - 1, t)), s = this.#r[e];
    return s < 0 || !this.#c(e, s) ? !1 : (this.#i[e + s * this.#s] = 1, this.#r[e] = s - 1, !0);
  }
  /** Simulate several random landings per frame so the pile grows without the snow renderer. */
  #f() {
    const t = this.#s;
    if (!(t <= 0))
      for (let e = 0; e < it; e++) {
        const s = Math.floor(Math.random() * t);
        this.#h(s);
      }
  }
  start(t) {
    const e = t ?? document.body;
    this.#t = document.createElement("canvas"), this.#t.id = "umb-christmas-snow-accumulation", Object.assign(this.#t.style, {
      position: "fixed",
      bottom: "0",
      left: "0",
      width: "100vw",
      height: d + "px",
      pointerEvents: "none",
      zIndex: "0",
      mixBlendMode: "lighten"
    }), e.appendChild(this.#t), this.#o = document.createElement("div"), this.#o.id = "umb-christmas-santa-path";
    const s = document.createElement("div");
    s.id = "santa", this.#o.appendChild(s), e.appendChild(this.#o), this.#l = document.createElement("style"), this.#l.id = "umb-christmas-santa-path-styles";
    let n = "";
    if (v.length > 0 && (n = `:root { --umb-santa-sprite: url('${v.startsWith("data:") ? v : `data:image/png;base64,${v}`}'); }
`), this.#l.textContent = n + et, document.head.appendChild(this.#l), this.#e = this.#t.getContext("2d", { willReadFrequently: !1, alpha: !0 }), !this.#e) {
      this.#t.remove(), this.#t = null;
      return;
    }
    this.#d(), window.addEventListener("resize", this.#m), this.#n = requestAnimationFrame(this.#V);
  }
  stop() {
    this.#n !== null && cancelAnimationFrame(this.#n), window.removeEventListener("resize", this.#m), this.#o?.remove(), this.#o = null, this.#l?.remove(), this.#l = null, this.#t?.remove(), this.#t = null, this.#e = null, this.#i = null, this.#r = null, this.#a = null;
  }
  #m = () => this.#d();
  #d() {
    if (!this.#t || !this.#e) return;
    const t = Math.max(1, innerWidth);
    this.#s = t, this.#t.width = t, this.#t.height = d, this.#i = new Uint8Array(t * d), this.#r = new Int32Array(t);
    for (let e = 0; e < t; e++) this.#r[e] = d - 1;
    for (let e = 0; e < t; e++) this.#i[e + R * t] = 1;
    for (let e = 0; e < t; e++) this.#r[e] = R - 1;
    this.#a = this.#e.getImageData(0, 0, t, d);
  }
  #V = (t) => {
    this.#n = requestAnimationFrame((s) => this.#V(s));
    const e = t ?? performance.now();
    e - this.#A < rt || (this.#A = e, this.#f(), this.#u());
  };
  #u() {
    const t = this.#e, e = this.#a?.data, s = this.#i;
    if (!t || !e || !s || this.#s <= 0) return;
    const r = this.#s * d * 4;
    for (let l = 0; l < r; l += 4)
      e[l] = 0, e[l + 1] = 0, e[l + 2] = 0, e[l + 3] = 0;
    for (let l = 0; l < s.length; l++) {
      if (s[l] !== 1) continue;
      const a = l * 4;
      e[a] = st, e[a + 1] = nt, e[a + 2] = ot, e[a + 3] = 220;
    }
    const h = this.#a;
    h && this.#e && this.#e.putImageData(h, 0, 0);
  }
}
const w = "/App_Plugins/BackofficeThemes/santahatlogo.svg";
class lt {
  #t = null;
  #e = null;
  #n = null;
  #s = null;
  #i = 0;
  start() {
    this.#r();
  }
  stop() {
    this.#A(), this.#n?.disconnect(), this.#n = null, this.#s && clearTimeout(this.#s);
  }
  #r() {
    const t = this.#a();
    t ? this.#l(t) : this.#i < 30 && (this.#i++, this.#s = setTimeout(() => this.#r(), 500));
  }
  #a() {
    try {
      const t = document.querySelector("umb-app");
      if (!t?.shadowRoot) return null;
      let e = t.shadowRoot.querySelector("umb-backoffice");
      if (e || (e = t.shadowRoot.querySelector("umb-router-slot")?.shadowRoot?.querySelector("umb-backoffice")), e || (e = this.#o(t.shadowRoot, "umb-backoffice")), !e?.shadowRoot) return null;
      const s = e.shadowRoot.querySelector("umb-backoffice-header");
      if (!s?.shadowRoot) return null;
      const n = s.shadowRoot.querySelector("umb-backoffice-header-logo");
      if (!n?.shadowRoot) return null;
      const o = n.shadowRoot.querySelector("umb-app-logo");
      return o ? o.querySelector("img") || o.shadowRoot?.querySelector("img") : null;
    } catch {
      return null;
    }
  }
  #o(t, e) {
    const s = t.querySelector(e);
    if (s) return s;
    for (const n of Array.from(t.querySelectorAll("*")))
      if (n.shadowRoot) {
        const o = this.#o(n.shadowRoot, e);
        if (o) return o;
      }
    return null;
  }
  #l(t) {
    t.src.includes(w) || (this.#e = t, this.#t = t.src, t.src = w, this.#n = new MutationObserver(() => {
      t.src !== w && !t.src.includes(w) && (t.src = w);
    }), this.#n.observe(t, { attributes: !0, attributeFilter: ["src"] }));
  }
  #A() {
    this.#e && this.#t && (this.#e.src = this.#t), this.#e = null, this.#t = null;
  }
}
const q = 60, y = 12, m = 4, D = 50, At = 1e3 / 24, M = ["#ff3333", "#33cc55", "#ffcc00", "#3388ff", "#ff66cc", "#ffffff"];
class ht {
  #t = null;
  #e = null;
  #n = null;
  #s = [];
  #i = 0;
  #r = 0;
  start() {
    const t = q + y + m * 2 + 8;
    if (this.#t = document.createElement("canvas"), this.#t.id = "umb-christmas-lights", Object.assign(this.#t.style, {
      position: "fixed",
      top: "0",
      left: "0",
      width: "100vw",
      height: t + "px",
      pointerEvents: "none",
      zIndex: "99998"
    }), document.body.appendChild(this.#t), this.#e = this.#t.getContext("2d", { willReadFrequently: !1 }), !this.#e) {
      this.#t.remove();
      return;
    }
    this.#o(), this.#l(), window.addEventListener("resize", this.#a), this.#i = performance.now(), this.#A();
  }
  stop() {
    this.#n && cancelAnimationFrame(this.#n), window.removeEventListener("resize", this.#a), this.#t?.remove(), this.#t = null, this.#e = null, this.#s = [];
  }
  #a = () => {
    this.#o(), this.#l();
  };
  #o() {
    if (!this.#t || !this.#e) return;
    const t = devicePixelRatio || 1, e = innerWidth, s = q + y + m * 2 + 8;
    this.#t.width = e * t, this.#t.height = s * t, this.#t.style.width = e + "px", this.#t.style.height = s + "px", this.#e.setTransform(t, 0, 0, t, 0, 0);
  }
  #l() {
    this.#s = [];
    let t = 0;
    const e = 300;
    for (let s = D / 2; s < innerWidth; s += D) {
      const n = s % e / e * Math.PI;
      this.#s.push({
        x: s,
        y: q + Math.sin(n) * y,
        color: M[t++ % M.length],
        phase: Math.random() * Math.PI * 2,
        speed: 0.5 + Math.random() * 1.5
      });
    }
  }
  #A = (t) => {
    this.#n = requestAnimationFrame((s) => this.#A(s));
    const e = t ?? performance.now();
    e - this.#r < At || (this.#r = e, this.#c());
  };
  #c() {
    const t = this.#e;
    if (!t) return;
    const e = innerWidth, s = q + y + m * 2 + 8, n = (performance.now() - this.#i) / 1e3;
    if (t.clearRect(0, 0, e, s), !!this.#s.length) {
      t.beginPath(), t.strokeStyle = "#2a3a20", t.lineWidth = 1.5, t.moveTo(0, q);
      for (let o = 0; o < this.#s.length; o++) {
        const r = this.#s[o];
        if (o === 0)
          t.lineTo(r.x, r.y);
        else {
          const h = this.#s[o - 1];
          t.quadraticCurveTo((h.x + r.x) / 2, Math.max(h.y, r.y) + 2, r.x, r.y);
        }
      }
      t.lineTo(e, q), t.stroke();
      for (const o of this.#s) {
        const r = Math.sin(n * o.speed * 2 + o.phase), h = r > 0.2 ? 1 : r > -0.2 ? (r + 0.2) / 0.4 : 0.1;
        t.fillStyle = "#333", t.fillRect(o.x - 1.5, o.y - 2, 3, 4);
        const l = t.createRadialGradient(o.x, o.y + 3, 0, o.x, o.y + 3, m * 3);
        l.addColorStop(0, this.#h(o.color, 0.3 * h)), l.addColorStop(1, "transparent"), t.beginPath(), t.fillStyle = l, t.arc(o.x, o.y + 3, m * 3, 0, Math.PI * 2), t.fill(), t.beginPath(), t.fillStyle = this.#h(o.color, 0.6 + 0.4 * h), t.ellipse(o.x, o.y + 3 + m * 0.3, m * 0.8, m, 0, 0, Math.PI * 2), t.fill(), t.beginPath(), t.fillStyle = `rgba(255,255,255,${0.3 * h})`, t.arc(o.x - 1, o.y + 2, m * 0.35, 0, Math.PI * 2), t.fill();
      }
    }
  }
  #h(t, e) {
    const s = parseInt(t.slice(1, 3), 16), n = parseInt(t.slice(3, 5), 16), o = parseInt(t.slice(5, 7), 16);
    return `rgba(${s},${n},${o},${e})`;
  }
}
const E = "umb-christmas-shadow-snow", F = "umb-christmas-shadow-snow-doc", W = `
	content: '';
	position: absolute;
	width: 36px;
	height: 12px;
	background: linear-gradient(180deg,
		rgba(255,255,255,0.98) 0%,
		rgba(255,255,255,0.9) 25%,
		rgba(248,252,255,0.85) 50%,
		rgba(235,245,255,0.7) 100%);
	border-radius: 50% 50% 30% 30% / 85% 85% 0 0;
	box-shadow: 0 1px 2px rgba(0,0,0,0.08), inset 0 1px 0 rgba(255,255,255,0.6);
	mix-blend-mode: lighten;
	pointer-events: none;
	z-index: 1;
`, ct = `
:host(uui-tab-group),
uui-tab-group {
	position: relative !important;
	overflow: visible !important;
}
:host(uui-tab-group)::after,
uui-tab-group::after {
	${W}
	bottom: 0;
	right: 4px;
}

:host(umb-body-layout),
umb-body-layout {
	position: relative !important;
	overflow: visible !important;
}
:host(umb-body-layout)::after,
umb-body-layout::after {
	${W}
	bottom: 0;
	left: 8px;
}

:host(umb-workspace-footer),
umb-workspace-footer {
	position: relative !important;
	overflow: visible !important;
}
:host(umb-workspace-footer)::after,
umb-workspace-footer::after {
	${W}
	bottom: 0;
	right: 8px;
	width: 32px;
	height: 10px;
}

:host(umb-footer-layout),
umb-footer-layout {
	position: relative !important;
	overflow: visible !important;
}
:host(umb-footer-layout)::before,
umb-footer-layout::before {
	${W}
	bottom: 0;
	left: 8px;
	width: 28px;
	height: 10px;
}
`, mt = "data:text/css;charset=utf-8," + encodeURIComponent(ct);
function Y(i) {
  const t = (e) => {
    e instanceof Element && e.shadowRoot && (i(e.shadowRoot), t(e.shadowRoot));
    for (const s of e.childNodes) t(s);
  };
  t(document.documentElement);
}
function ft(i) {
  if (i.getElementById(E)) return;
  const t = document.createElement("link");
  t.id = E, t.setAttribute("data-umb-christmas-snow", "true"), t.rel = "stylesheet", t.href = mt, i.appendChild(t);
}
function dt(i) {
  i.getElementById(E)?.remove();
}
class Vt {
  #t = null;
  #e = [];
  #n() {
    Y(ft);
  }
  start() {
    document.documentElement.setAttribute("data-umb-christmas-snow", "true"), this.#s(), this.#n(), [100, 400, 1e3, 2500].forEach((t) => {
      this.#e.push(setTimeout(() => this.#n(), t));
    }), this.#t = new MutationObserver(() => this.#n()), this.#t.observe(document.documentElement, {
      childList: !0,
      subtree: !0
    });
  }
  stop() {
    this.#e.forEach((t) => clearTimeout(t)), this.#e = [], this.#t?.disconnect(), this.#t = null, Y(dt), document.getElementById(F)?.remove(), document.documentElement.removeAttribute("data-umb-christmas-snow");
  }
  #s() {
    if (document.getElementById(F)) return;
    const t = 'content:"";position:absolute;background:linear-gradient(180deg,rgba(255,255,255,.98) 0%,rgba(255,255,255,.9) 25%,rgba(248,252,255,.85) 50%,rgba(235,245,255,.7) 100%);border-radius:50% 50% 30% 30%/85% 85% 0 0;box-shadow:0 1px 2px rgba(0,0,0,.08),inset 0 1px 0 rgba(255,255,255,.6);mix-blend-mode:lighten;pointer-events:none;z-index:1;', e = document.createElement("style");
    e.id = F, e.textContent = `
			[data-umb-christmas-snow] uui-tab-group { position: relative !important; overflow: visible !important; }
			[data-umb-christmas-snow] uui-tab-group::after { ${t} bottom: 0; right: 4px; width: 36px; height: 12px; }
			[data-umb-christmas-snow] umb-body-layout { position: relative !important; overflow: visible !important; }
			[data-umb-christmas-snow] umb-body-layout::after { ${t} bottom: 0; left: 8px; width: 36px; height: 12px; }
			[data-umb-christmas-snow] umb-workspace-footer { position: relative !important; overflow: visible !important; }
			[data-umb-christmas-snow] umb-workspace-footer::after { ${t} bottom: 0; right: 8px; width: 32px; height: 10px; }
			[data-umb-christmas-snow] umb-footer-layout { position: relative !important; overflow: visible !important; }
			[data-umb-christmas-snow] umb-footer-layout::before { ${t} bottom: 0; left: 8px; width: 28px; height: 10px; }
		`, document.head.appendChild(e);
  }
}
const ut = 100, z = 140, pt = 16, gt = 16, I = 64, qt = 8, O = "1rem";
function wt(i) {
  const t = i * Math.PI / 180;
  return Math.sin(t);
}
function Ct(i) {
  const t = i * Math.PI / 180;
  return Math.cos(t);
}
class Kt {
  #t = null;
  #e = null;
  start() {
    this.#t = document.createElement("div"), this.#t.id = "umb-christmas-tree", Object.assign(this.#t.style, {
      position: "fixed",
      bottom: pt + "px",
      left: gt + "px",
      width: ut + "px",
      height: z + "px",
      pointerEvents: "none",
      zIndex: "2147483645",
      overflow: "hidden"
    });
    const t = document.createElement("ul");
    t.className = "umb-christmas-tree-star";
    for (let s = 0; s < 5; s++) {
      const n = document.createElement("li");
      n.style.transform = `rotate(${s * 72}deg)`, t.appendChild(n);
    }
    this.#t.appendChild(t);
    const e = document.createElement("ul");
    e.className = "umb-christmas-tree-lines";
    for (let s = 0; s < I; s++) {
      const n = document.createElement("li"), o = 6 * 360 * s / I, r = (wt(o) + Ct(o / 3) * 0.1) * 20;
      n.style.transform = `rotate(${r}deg)`, n.style.setProperty("--umb-tree-delay", `${-s * (0.5 * qt) / I}s`), e.appendChild(n);
    }
    this.#t.appendChild(e), this.#e = document.createElement("style"), this.#e.id = "umb-christmas-tree-styles", this.#e.textContent = `
			#umb-christmas-tree { box-sizing: border-box; }
			#umb-christmas-tree *, #umb-christmas-tree *::before, #umb-christmas-tree *::after { box-sizing: border-box; }
			#umb-christmas-tree ul, #umb-christmas-tree li { list-style: none; margin: 0; padding: 0; }
			.umb-christmas-tree-star {
				position: absolute;
				top: 12px;
				left: 50%;
				width: 1rem;
				height: 1rem;
				transform: translate(-50%, -50%);
			}
			.umb-christmas-tree-star li {
				position: absolute;
				width: 0;
				height: 0;
				overflow: hidden;
				border-width: 0 0.35rem 0.7rem 0.35rem;
				border-style: solid;
				border-top-color: transparent;
				border-left-color: transparent;
				border-right-color: transparent;
				border-bottom-color: #FFCE54;
				transform-origin: 0.35rem 0.7rem;
			}
			.umb-christmas-tree-lines {
				position: absolute;
				top: 0;
				left: 50%;
				width: 0;
				height: 100%;
				transform: translateX(-50%);
			}
			.umb-christmas-tree-lines li {
				position: absolute;
				top: 28px;
				left: 50%;
				width: 0;
				height: ${z - 28}px;
				background: transparent;
				transform-origin: 50% 0;
			}
			.umb-christmas-tree-lines li::before,
			.umb-christmas-tree-lines li::after {
				content: "";
				position: absolute;
				left: 50%;
				animation: umb-christmas-tree-fall 4s linear infinite;
				animation-delay: var(--umb-tree-delay, 0s);
			}
			.umb-christmas-tree-lines li::before {
				top: 0;
				width: 1px;
				height: ${O};
				background: linear-gradient(rgba(46,204,113,0), rgba(46,204,113,0.5));
				transform: translateX(-50%);
			}
			.umb-christmas-tree-lines li::after {
				top: ${O};
				width: 3px;
				height: 3px;
				border-radius: 100%;
				transform: translate(-50%, 0);
			}
			.umb-christmas-tree-lines li:nth-child(4n)::after { background: #D8334A; }
			.umb-christmas-tree-lines li:nth-child(4n+1)::after { background: #FFCE54; }
			.umb-christmas-tree-lines li:nth-child(4n+2)::after { background: #2ECC71; }
			.umb-christmas-tree-lines li:nth-child(4n+3)::after { background: #5D9CEC; }
			@keyframes umb-christmas-tree-fall {
				0% { opacity: 0; top: 0; }
				5% { opacity: 0; }
				15%, 90% { opacity: 1; }
				100% { opacity: 0; top: 100%; }
			}
		`, document.head.appendChild(this.#e), document.body.appendChild(this.#t);
  }
  stop() {
    this.#t?.remove(), this.#t = null, this.#e?.remove(), this.#e = null;
  }
}
const C = 6, N = 28, Z = 6, K = 10, H = ["#c41e3a", "#ffd700", "#228b22", "#1e90ff", "#c41e3a", "#ffd700"];
function bt() {
  const i = window.location.pathname;
  return i.includes("/modal/") || i.includes("/block/edit/") || i.includes("/view/");
}
class Bt {
  #t = null;
  #e = null;
  #n = null;
  #s = /* @__PURE__ */ new WeakSet();
  #i = "";
  #r() {
    const t = (e) => {
      if (e instanceof Element && e.shadowRoot) {
        const s = e.shadowRoot;
        s.getElementById("umb-christmas-workspace-balls")?.remove(), s.querySelectorAll("umb-body-layout").forEach((n) => {
          n.shadowRoot?.getElementById("umb-christmas-workspace-balls")?.remove();
        }), t(s);
      }
      for (const s of e.childNodes) t(s);
    };
    t(document.documentElement), this.#s = /* @__PURE__ */ new WeakSet();
  }
  #a(t = !1) {
    const e = window.location.pathname;
    if (!bt()) {
      this.#o(), this.#r(), this.#i = e;
      return;
    }
    this.#i = e, this.#r();
    const s = [], n = (o) => {
      if (o instanceof Element && o.shadowRoot) {
        if (o.tagName === "UMB-WORKSPACE-EDITOR") {
          const r = o.shadowRoot.querySelector("umb-body-layout");
          r?.shadowRoot && r.shadowRoot.getElementById("header") && s.push(r.shadowRoot);
        }
        n(o.shadowRoot);
      }
      for (const r of o.childNodes) n(r);
    };
    if (n(document.documentElement), s.length === 1 && !t) {
      this.#n === null && (this.#n = setTimeout(() => {
        this.#n = null, this.#a(!0);
      }, 220));
      return;
    }
    s.length > 0 && this.#l(s[s.length - 1]);
  }
  #o() {
    this.#n !== null && (clearTimeout(this.#n), this.#n = null);
  }
  #l(t) {
    if (this.#s.has(t)) return;
    const e = t.getElementById("header");
    if (!e || e.querySelector("#umb-christmas-workspace-balls")) return;
    const s = document.createElement("div");
    s.id = "umb-christmas-workspace-balls", Object.assign(s.style, {
      position: "absolute",
      top: "0",
      left: "0",
      right: "0",
      height: K + Z + C * 2 + 4 + "px",
      pointerEvents: "none",
      overflow: "hidden"
    });
    const n = document.createElement("canvas");
    s.appendChild(n);
    const o = n.getContext("2d");
    if (!o) {
      s.remove();
      return;
    }
    e.style.position = "relative", e.appendChild(s);
    const r = () => {
      const a = e.offsetWidth, f = K + Z + C * 2 + 4, u = devicePixelRatio || 1;
      n.width = a * u, n.height = f * u, n.style.width = a + "px", n.style.height = f + "px", o.setTransform(u, 0, 0, u, 0, 0), h(o, a, f);
    }, h = (a, f, u) => {
      a.clearRect(0, 0, f, u);
      const p = [];
      let j = 0;
      const k = 200;
      for (let A = N / 2; A < f; A += N) {
        const c = A % k / k * Math.PI;
        p.push({
          x: A,
          y: K + Math.sin(c) * Z,
          color: H[j++ % H.length]
        });
      }
      if (p.length !== 0) {
        a.strokeStyle = "#3d4a36", a.lineWidth = 1.2, a.beginPath(), a.moveTo(0, K);
        for (let A = 0; A < p.length; A++) {
          const c = p[A];
          if (A === 0) a.lineTo(c.x, c.y);
          else {
            const g = p[A - 1];
            a.quadraticCurveTo((g.x + c.x) / 2, Math.max(g.y, c.y) + 2, c.x, c.y);
          }
        }
        a.lineTo(f, K), a.stroke();
        for (const A of p) {
          const c = A.x, g = A.y + C * 0.4, b = a.createRadialGradient(
            c - 2,
            g - 2,
            0,
            c,
            g,
            C
          );
          b.addColorStop(0, "rgba(255,255,255,0.7)"), b.addColorStop(0.25, A.color), b.addColorStop(1, vt(A.color, 0.5)), a.beginPath(), a.fillStyle = b, a.arc(c, g, C, 0, Math.PI * 2), a.fill(), a.strokeStyle = "rgba(0,0,0,0.15)", a.lineWidth = 0.8, a.stroke();
        }
      }
    };
    new ResizeObserver(() => r()).observe(e), r(), this.#s.add(t);
  }
  #A = () => {
    window.location.pathname !== this.#i && this.#a();
  };
  start() {
    this.#a(), this.#t = new MutationObserver(() => this.#a()), this.#t.observe(document.documentElement, {
      childList: !0,
      subtree: !0
    }), [200, 600, 1500, 3e3, 5e3].forEach((s) => setTimeout(() => this.#a(), s)), window.addEventListener("popstate", this.#A);
    const t = history.pushState, e = history.replaceState;
    history.pushState = (...s) => {
      t.apply(history, s), this.#A();
    }, history.replaceState = (...s) => {
      e.apply(history, s), this.#A();
    }, this.#e = () => {
      window.removeEventListener("popstate", this.#A), history.pushState = t, history.replaceState = e;
    };
  }
  stop() {
    this.#o(), this.#t?.disconnect(), this.#t = null, this.#e?.(), this.#e = null, this.#r();
  }
}
function vt(i, t) {
  const e = Math.round(parseInt(i.slice(1, 3), 16) * t), s = Math.round(parseInt(i.slice(3, 5), 16) * t), n = Math.round(parseInt(i.slice(5, 7), 16) * t);
  return `rgb(${e},${s},${n})`;
}
const yt = "umb-christmas-theme";
let U = null, x = null, V = null, S = null, X = null, T = null, Q = null, L = null, P = !1, J = null;
function Wt() {
  P || (P = !0, console.log("[ChristmasTheme] 🎄 Ho ho ho! Starting Christmas effects!"), V = document.createElement("div"), Object.assign(V.style, {
    position: "fixed",
    inset: "0",
    pointerEvents: "none",
    zIndex: "2147483646"
  }), document.body.appendChild(V), x = new at(), x.start(V), U = new tt(), U.start(void 0, V), S = new lt(), S.start(), X = new ht(), X.start(), T = new Vt(), T.start(), Q = new Kt(), Q.start(), L = new Bt(), L.start());
}
function G() {
  P && (P = !1, console.log("[ChristmasTheme] Stopping Christmas effects"), U?.stop(), U = null, x?.stop(), x = null, V?.remove(), V = null, S?.stop(), S = null, X?.stop(), X = null, T?.stop(), T = null, Q?.stop(), Q = null, L?.stop(), L = null);
}
function Ut(i) {
  i === yt ? Wt() : G();
}
const St = (i) => {
  i.getContext(_).then((t) => {
    t && (J = t.theme.subscribe((e) => {
      Ut(e);
    }));
  });
}, Xt = () => {
  J?.unsubscribe(), J = null, G();
};
export {
  St as onInit,
  Xt as onUnload
};
//# sourceMappingURL=effects.entrypoint-C7K7z-8f.js.map
